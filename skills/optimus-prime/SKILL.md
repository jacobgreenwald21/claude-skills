...
> Note: The canonical skill list lives at `~/Desktop/AI/claude-skills/`. Reference this repo when doing cross-skill analysis to ensure no skills are missed.
...
3. **Session log** — read `~/.claude/projects/-Users-jacob/memory/session-log.md` if it exists. Look specifically for: audit flags that appear across multiple sessions (recurring issues), skills that are repeatedly modified or rebuilt, misfires that show up more than once, and friction points that suggest a missing or broken skill. This is the primary source for cross-session pattern data.
4. **CLAUDE.md** — read the skill portfolio list and any usage notes there.
