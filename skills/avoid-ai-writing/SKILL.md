...
### "Related to" padding
- AI defaults to "related to" as a generic connector between a noun and its modifier instead of picking the specific relationship: "issues related to onboarding," "questions related to pricing," "challenges related to scale." The phrase does no work — swap in the specific preposition, verb, or compound that actually describes the relationship.
- Flag at 2+ uses in the same piece. One instance is ordinary English; AI reaches for it repeatedly as a default bridge where a person would vary the construction or just use a compound noun.
- Fix: name the actual relationship. "Related to" → "about," "on," "involving," "tied to," "behind," "from," or restructure as a compound ("pricing questions" instead of "questions related to pricing").
...
### Colon-forced modifiers
- AI habitually appends a colon and a defining, exemplifying, or restating clause to a sentence that already stood on its own: "The team prioritized speed: shipping updates weekly instead of quarterly." The clause after the colon often just narrows or restates the first half, and the sentence reads the same — sometimes better — without it.
- Test: delete the colon and everything after it. If the sentence still carries the point, the colon was decorative, not structural. Cut the trailing clause, or fold its content back into the main clause.
- If the trailing content is a genuine new example or a short list, don't force it onto a colon splice — use "such as," "for example," "including," or write it as its own sentence. "The team fixed the bug: updating the validator and adding a test." reads better as "The team fixed the bug by updating the validator and adding a test" or "...fixed the bug, such as updating the validator."
- Distinct from **List-label periods** above, which fixes the opposite failure — a bullet-list label wrongly punctuated with a period instead of a colon. That rule governs bullet labels; this one catches a colon force-fit into ordinary prose where the sentence didn't need it.
...
- Colon-forced modifiers (unearned colon splice mid-sentence)
...
- "Related to" padding (2+ uses in one piece)
